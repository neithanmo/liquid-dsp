...
impl Fec {
    ...
    pub fn encode(&self, raw: &[u8], encoded: &mut [u8]) {
        unsafe {
            raw::fec_encode(self.inner, raw.len() as c_uint, raw.as_ptr() as *mut _, encoded.as_mut_ptr() as _);
        }
    }
    ...
    pub fn decode(&self, encoded: &[u8], raw: &mut [u8]) {
        unsafe {
            raw::fec_decode(self.inner, raw.len() as c_uint, encoded.as_ptr() as _, raw.as_mut_ptr() as _);
        }
    }

    pub fn decode_soft(&self, encoded: &[u8], raw: &mut [u8]) {
        unsafe {
            raw::fec_decode_soft(self.inner, raw.len() as c_uint, encoded.as_ptr() as _, raw.as_mut_ptr() as _);
        }
    }
